  ...
  async up(db, client) {
    ...
    //   branches.map(async branch => {
      // 639e9c4d1ccc1d5fd9f319b4  
        const groups = await db.collection('groups').find({ _id: ObjectId('639e860636491596e49eda65') }).toArray();    // {email: "lo4mabalacat@ambercashph.com"}
        // const groups = await db.collection('groups').find({ loanOfficerId: "63a073a99c181cef6b0506d2" }).toArray(); // 
        if (groups) {
          groups.map(group => {
