...
const ViewByAreaPage = ({dateFilter, type, selectedBranchGroup, viewMode}) => {
    ...
    const [areaCollectionData, setAreaCollectionData] = useState([]);
    ...
    const getCollectionData = async (date) => {
        ...
        const response = await fetchWrapper.get(process.env.NEXT_PUBLIC_API_URL + 
                            'transactions/cash-collections/get-all-loans-per-area?' 
        ...
        if (response.success) {
            ...
            setAreaCollectionData(collectionData);
            setLoading(false);
        }
    }
    ...
    return (
        <React.Fragment>
            {loading ?
                ...
                ) : <TableComponent columns={columns} data={areaCollectionData} hasActionButtons={false} rowActionButtons={false} showFilters={false} showPagination={false} pageSize={100} />}
        </React.Fragment>
    )
}

export default ViewByAreaPage;