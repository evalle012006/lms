...
const AddUpdateLoan = ({ mode = 'add', loan = {}, showSidebar, setShowSidebar, onClose, type }) => {
    ...
    const handlePNNumber = async (e) => {
        ...
        if (pnNumber && currentBranch) {
            const response = await fetchWrapper.get(process.env.NEXT_PUBLIC_API_URL + 'transactions/loans/check-existing-pn-number-by-branch?' + new URLSearchParams({ branchId: currentBranch._id, pnNumber: pnNumber, currentDate: currentDate }));
            if (response.success) {
