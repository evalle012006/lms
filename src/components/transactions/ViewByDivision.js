...
const ViewByDivisionPage = ({dateFilter, type, selectedBranchGroup, viewMode}) => {
    ...
    const getCollectionData = async (date) => {
        ...
        if (response.success) {
            ...
            setDivisionCollectionData(collectionData);
            setLoading(false);
        }
    }
    ...
    return (
        <React.Fragment>
            {loading ?
                ...
                ) : <TableComponent columns={columns} data={divisionCollectionData} hasActionButtons={false} rowActionButtons={false} showFilters={false} showPagination={false} pageSize={100} />}
        </React.Fragment>
    )
}

export default ViewByDivisionPage;