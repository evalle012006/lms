...
import { UppercaseFirstLetter, formatPricePhp, safeNumber } from "@/lib/utils";
...
const ViewCashCollectionPage = ({ pageNo, dateFilter, type }) => {
    ...
    const getCashCollections = async (dateFilter) => {
        ...
        if (response.success) {
            ...
            responseData && responseData.map(cc => {
                ...
                collection.totalNetCollectionStr = totalNetCollection > 0 ? formatPricePhp(totalNetCollection) : '-';
            ...
            if (collectionData.length > 0) {
                ...
                if (transferGvr?.totalLoanRelease > 0) {
                    ...
                    totals.csfCollectionStr = formatPricePhp(totals.csfCollection);
                    totals.csfWithdrawal -= transferGvr.csfWithdrawal;
                    totals.csfWithdrawalStr = formatPricePhp(totals.csfWithdrawal);
                }
                ...
                if (transferRcv?.totalLoanRelease > 0) {
                    ...
                    totals.pastDueStr = formatPricePhp(totals.pastDue);
                    totals.csfCollection += transferRcv.csfCollection;
                    totals.csfCollectionStr = formatPricePhp(totals.csfCollection);
                    totals.csfWithdrawal += transferRcv.csfWithdrawal;
                    totals.csfWithdrawalStr = formatPricePhp(totals.csfWithdrawal);
                }
    ...
    const consolidateTotals = (totals, transferGvr, transferRcv) => {
        ...
        return {
            ...
            mcbuStr: formatPricePhp(totalMcbu),
            csf: totalsCsf,
            csfStr: formatPricePhp(totalsCsf),
            totalData: true,
            status: '-'
        }
    }
    ...
    const transferDetailsTotal = (details, type) => {
        ...
        return {
            ...
            csfStr: '-',
            csfCollection: totalCsfCollection,
            csfCollectionStr: (type === 'Transfer GVR' && totalCsfCollection > 0) ? `(${formatPricePhp(totalCsfCollection)})` : formatPricePhp(totalCsfCollection),
            csfWithdrawal: totalCsfWithdrawal,
    ...
    const createLos = (totals, selectedBranch, selectedLO, dateFilter, yearEnd, transferGvr, transferRcv, transferGvrByGroup, transferRcvByGroup, consolidateTotalData) => {
        ...
        let totalsCsfCollection = totals.csfCollection;
        ...
        if ((transferGvr?.totalLoanRelease > 0 && transferGvr.totalLoanRelease !== transferGvrByGroup.totalLoanRelease) 
            ...
            totalsCsfCollection = consolidateTotalData.csfCollection;
