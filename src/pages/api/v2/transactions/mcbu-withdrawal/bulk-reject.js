// transactions/mcbu-withdrawal/bulk-reject.js
...
async function bulkReject(req, res) {
  try {
    ...
    for (const withdrawal of withdrawals) {
      const { id, reason, modified_by, modified_date } = withdrawal;
      ...
      if (!reason) {
        errors.push({ error: true, message: "Rejection reason is required", withdrawal });
        continue;
      }
    ...
    return res.status(200).json({
      ...
      message: `Successfully rejected ${results.length} of ${withdrawals.length} withdrawals${errors.length > 0 ? ` (${errors.length} failed)` : ''}`,
      results,
    ...
    return res.status(500).json({
      error: true,
      message: "Failed to process bulk rejection: " + error.message
    });
  }
}