...
async function getData(req, res) {
    ...
    if (promise) {
        ...
        const processedData = processData(data, date, currentDate);
...
const processData = (data = [], dateFilter, currentDate) => {
  let collectionData = [];
  ...
  data.filter(rd => rd?.origin !== 'automation-trf').map(cc => {
    let collection = {
        groupId: cc._id,
        group: cc.name,
        groupNo: cc.groupNo,
    ...
    if (!filter) {
        ...
        if (cc?.draftCollections?.length > 0) {
            ...
            collection = { ...collection,
                ...
                mcbuInterest: draftCollection.mcbuInterest ? draftCollection.mcbuInterest: 0,
                mcbuInterestStr: draftCollection.mcbuInterest > 0 ? formatPricePhp(draftCollection.mcbuInterest): '-',
  ...
  let transferGvr = {};
  let transferRcv = {};
  let transferGvrByGroup = {};
  let transferRcvByGroup = {};
  let consolidateTotalData = {};

  if (collectionData.length > 0) {
    collectionTransferred.sort((a, b) => { return a.slotNo - b.slotNo; });
    collectionReceived.sort((a, b) => { return a.slotNo - b.slotNo; });
    collectionTransferredByGroup.sort((a, b) => { return a.slotNo - b.slotNo; });
    collectionReceivedByGroup.sort((a, b) => { return a.slotNo - b.slotNo; });
    transferGvr = transferDetailsTotal(collectionTransferred, 'Transfer GVR');
    transferRcv = transferDetailsTotal(collectionReceived, 'Transfer RCV');
    transferGvrByGroup = transferDetailsTotal(collectionTransferredByGroup, 'Transfer GVR');
    transferRcvByGroup = transferDetailsTotal(collectionReceivedByGroup, 'Transfer RCV');
    ...
    consolidateTotalData = consolidateTotals(totals, transferGvr, transferRcv);
  ...
  return { 
    ...
    collectionReceived, 
    collectionTransferredByGroup, 
    collectionReceivedByGroup, 
    transferGvrByGroup,
    transferRcvByGroup,
    transferGvr, 
    transferRcv,
    consolidateTotalData,
  };
