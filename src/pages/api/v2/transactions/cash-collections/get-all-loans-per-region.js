    ...
    if (user?.length > 0) {
        ...
        if (selectedBranchGroup === 'mine' && user[0].role.rep !== 1) {
            if (user[0].regionId && user[0].role.shortCode === 'regional_manager') {
                const regions = await findRegions({ _id: { _eq: user[0].regionId } });
                regionIds = regions.map(region => region._id.toString());
            } else if (user[0].divisionId && user[0].role.shortCode === 'deputy_director') {
                const regions = await findRegions({ divisionId: { _eq: user[0].divisionId } });
                regionIds = regions.map(region => region._id.toString());
            }
        } else {
        ...
        const result = await Promise.all(regionIds.map(async (regionId) => {
            ...
            data.push.apply(data, await getAllLoanTransactionsByRegion(regionId, date, dayName, currentDate));
        }));
...
async function getAllLoanTransactionsByRegion(regionId, date, dayName, currentDate) {
    let cashCollection;
    ...
    if (currentDate === date) {
      ...
      `;
      const variables = { regionId, date, dayName };
        ...
        region?.branchCollection?.map(branch => {
            ...
            if (!filter) {
                ...
                if (branch.cashCollections?.length > 0) {
                    ...
                    const otherIncome = branch.cashCollections[0].otherIncome == 0 ?
                                    (branch.cashCollections[0].otherPassbookCollection || 0) + (branch.cashCollections[0].otherPictureCollection || 0)
                                    : branch.cashCollections[0].otherIncome;
                    branchTotalOtherIncome += otherIncome;
                }
            }
