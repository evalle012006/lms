...
import { generateUUID, getEndDate } from '@/lib/utils';
...
const processDailyLOR = async (sheetData, branchId, loId, occurence) => {
    ...
    const groups = await graph.query(
        queryQl(GROUP_TYPE, { where: { loanOfficerId: { _eq: loId } } })
        ...
        const promises = clients.map(async c => {
            ...
            if (group) {
                ...
                const [resp] = await graph.mutation(
                    insertQl(createGraphType('client', `_id`)('client'), {
                        objects: [{ ... client, _id: generateUUID() }]
                    })
                ).then(res => res.data.client.returning)
                ...
                if (resp.acknowledged) {
                    ...
                    if (loan) {
                        ...
                        const [loanResp] = await graph.mutation(
                            insertQl(LOAN_TYPE, {
                                objects: [{ ... loan, _id: generateUUID() }]
                            })
                        ).then(resp => resp.data.loans.returning);

                        if (loanResp.acknowledged) {
                            loan._id = loanResp._id + '';
