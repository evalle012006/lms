    ...
    if (user.length > 0) {
        ...
        if (selectedBranchGroup == 'mine' && user[0].role.rep !== 1) {
            if (user[0].regionId && user[0].role.shortCode === 'regional_manager') {
                const regions = await db.collection('regions').find({ _id: new ObjectId(user[0].regionId) }).toArray();
                regionIds = regions.map(region => region._id.toString());
            } else if (user[0].divisionId && user[0].role.shortCode === 'deputy_director') {
                const regions = await db.collection('regions').find({ divisionId: user[0].divisionId }).toArray();
                regionIds = regions.map(region => region._id.toString());
            }
        } else {
        ...
        const promise = await new Promise(async (resolve) => {
            const response = await Promise.all(regionIds.map(async (regionId) => {
...
async function getAllLoanTransactionsByRegion(db, regionId, date, dayName, currentDate) {
    ...
    if (currentDate === date) {
        cashCollection = await db
            .collection('regions')
            .aggregate([
                { $match: { _id: new ObjectId(regionId) } },
