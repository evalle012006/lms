                ...
                .aggregate([
                    ...
                    },
                    {
                        $lookup: {
                            ...
                            foreignField: "clientId",
                            pipeline: [
                                { $addFields: { 'fullPaymentDateObj': {$dateFromString: { dateString: '$fullPaymentDate', format:"%Y-%m-%d" }}, 'currentDateObj': {$dateFromString: { dateString: date, format:"%Y-%m-%d" }} } },
                                { $match: {$expr: { $and: [
                                    {$eq: ['$status', 'completed']}, {$lte: ['$loanBalance', 0]} , {$lte: ['$startDateObj', '$currentDateObj']}
                                ]}} },
                                { $group: {
                                        _id: '$$clientId',
                                        fullPaymentAmount: { $sum: '$history.amountRelease' },
                    ...
                    { $project: { clientIdObj: 0, loanIdStr: 0, startDateObj: 0, groupIdObj: 0, fullPaymentDateObj: 0 } }
                ])
                .toArray();
        } else if (loId) {
            cashCollection = await db
                ...
                .aggregate([
                    ...
                    },
                    {
                        $lookup: {
                            ...
                            foreignField: "loanId",
                            pipeline: [
                                { $match: { mode: mode, dateAdded: date } },
                                { $group: { 
                                        _id: '$clientId',
                                        mispayments: { $sum: { $cond:{if: { $eq: ['$mispayment', true] }, then: 1, else: 0} } },
                                        loanTarget: { $sum: '$activeLoan' },
