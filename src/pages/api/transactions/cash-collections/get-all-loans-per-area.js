    ...
    if (user.length > 0) {
        ...
        if (selectedBranchGroup == 'mine' && user[0].role.rep !== 1) {
            if (user[0].regionId && user[0].role.shortCode === 'regional_manager') {
                ...
                const areas = await db.collection('areas').find({ divisionId: user[0].divisionId }).toArray();
                areaIds = areas.map(area => area._id.toString());
            }
        } else {
        ...
        const promise = await new Promise(async (resolve) => {
            const response = await Promise.all(areaIds.map(async (areaId) => {
