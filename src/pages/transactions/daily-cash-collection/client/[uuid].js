...
import { LOR_NO_CSF_IN_REMARKS, LOR_DAILY_REMARKS, getApiBaseUrl } from '@/lib/constants';
...
const CashCollectionDetailsPage = () => {
    ...
    const filterDateMonth = useMemo(() => moment(dateFilter).month() , [dateFilter])
                ...
                if ((cc?.transferred == true && cc.transferredDate == currentDate) || ((cc?.transfer == true && cc.transferDate == currentDate) && cc?.current?.length == 0)) {
                    ...
                    if (cc.status === "tomorrow" || cc.status === "pending") { // only when filter
                        ...
                        collection = {
                            ...
                            csf: safeNumber(cc.csf),
                            csfStr: safeNumber(cc.csf) > 0 ? formatPricePhp(cc.csf) : '-',
                            csfCollection: cc.csfCollection ? cc.csfCollection : 0,
                            csfCollectionStr: cc.client.groupLeader ? formatPricePhp(cc.csfCollection) : '-',
                            csfWithdrawal: 0,
                            csfWithdrawalStr: '-',
                            csfReturnAmt: 0,
                            csfReturnAmtStr: '-',
                        ...
                        if (cc.current.length > 0) {
                            ...
                            if (current) {
                                ...
                                collection.csfIn = current.csfIn ? current.csfIn : 0;
                ...
                if (currentLoan && currentLoan.status !== 'pending') {
                    ...
                    if (clientObj) {
                        let pendingTomorrow = {
                            ...
                            clientId: loan.clientId,
                            loId: loan.loId,
                            fullName: UppercaseFirstLetter(`${clientObj.lastName}, ${clientObj.firstName} ${clientObj.middleName ? clientObj.middleName : ''}`),
                            loanCycle: loan.loanCycle,
                            amountReleaseStr: '-',
                            loanBalanceStr: '-',
                            targetCollectionStr: '-',
                ...
                if (idx === index) {
                    ...
                    //     toast.error('Error occured. Please set remarks as Excused Advance Payment.');
                        ...
                        if (temp.status === "completed" && (remarks.value && !(remarks.value?.startsWith('offset') || remarks.value?.startsWith('reloaner')))) {
                            ...
                            temp.allowMcbuCollection = false;
    ...
    useEffect(() => {
        let cashCollections = addBlankAndTotal(JSON.parse(JSON.stringify(groupClients)));
                            ...
                            <table className="table-auto border-collapse text-sm">
                                <thead className="border-b border-b-gray-300">
                                    <tr className="sticky top-0 column py-0 pr-0 pl-4 text-left text-gray-500 uppercase tracking-wider bg-white">
                                        ...
                                        {filterDateMonth === 11 && (<th className="p-2 text-center">MCBU Interest</th>)}
