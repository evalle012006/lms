...
// ── PromotedClientBanner ──────────────────────────────────────────────────
// Shown when a promoted client's QR is scanned.
// Fetches their latest loan and shows a contextual action button.
const PromotedClientBanner = ({ application, currentUser }) => {
    ...
    const handleAction = () => {
        if (loanId && (loanState === 'pending' || loanState === 'active')) {
            // Has a loan — go directly to edit page
            router.push(`/transactions/loan-applications/edit/${loanId}`);
        } else {
            // No loan yet — go to loan applications list filtered to pending clients
            // Staff adds the loan from there by selecting the client
            router.push('/transactions/loan-applications');
        }
    };
    ...
    return (
        <div className={`p-4 border rounded-xl ${cm.wrap}`}>
            <div className="flex items-center justify-between gap-3">
                <div>
                    <p className={`text-sm font-semibold ${cm.title}`}>
                        ✓ Promoted to Client
                    </p>
                ...
                {loanState !== null && (
                    <button type="button" onClick={handleAction}
                        className={`flex-shrink-0 px-3 py-1.5 text-white text-xs
                            font-semibold rounded-lg transition-colors ${cm.btn}`}>
                        {cfg.label}
                    </button>
                )}
            </div>
